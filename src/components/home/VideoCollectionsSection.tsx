...
export function VideoCollectionsSection({ collections }: VideoCollectionsSectionProps) {
  return (
    <section className="rounded-2xl border border-white/15 bg-white/[0.04] p-5 backdrop-blur-lg sm:p-6">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="text-2xl font-semibold text-white">Video Collections</h2>
          <p className="mt-1 text-sm text-zinc-300">
            Playlist-wise horizontal scroller from your CSV exports.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-5">
        {collections.map((collection) => (
          <article
            ...
            className="rounded-xl border border-white/15 bg-gradient-to-b from-white/10 to-white/[0.03] p-4"
          >
            <div className="mb-3 flex items-end justify-between gap-3">
              ...
              <p className="text-xs text-zinc-400">{collection.videos.length} videos</p>
            </div>
            <div className="flex gap-3 overflow-x-auto pb-2">
              {collection.videos.map((video) => (
                <div
                  key={video.url}
                  className="w-[280px] min-w-[280px] overflow-hidden rounded-lg border border-white/10"
                >
